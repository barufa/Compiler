...
fun topsort P =
  let 
    fun candidato E P = List.filter (fn e => List.all((op<> rs e) o snd) P) E
    fun tsort P [] Res = rev Res
      | tsort [] St Res = rev(St @ Res)
      | tsort P (St as (h::t)) Res = let val x = (hd(candidato St P)) handle Empty => raise Ciclo
...
fun buscaArrRecords lt =
  let 
    fun buscaRecs [] res = res
      | buscaRecs({name, ty=RecordTy lf}::t) res =
          let fun genrecs [] _ = []
                | genrecs({name, escape, typ=NameTy s}::tail) n = (name, ref(TTipo s), n)::genrecs tail (n+1)
  ...
  | procesa (sorted as (h::t)) (pares:{name:symbol, ty:ty} list) env recs =
    let
      ...
      val env' = case ttopt of
                  SOME tt => List.foldr (fn({name, ty=NameTy ty}, env') => tabRInserta(name, tt, env')
...
fun fijaNONE [] env = env
  ...
  | fijaNONE((name, TRecord(lf, u))::t) env =
    let 
      fun busNONE(s, ar as (ref(TTipo t)), _) = (ar := tabSaca(t, env) handle _ => raise noExisteS t)
        | busNONE _ = ()
