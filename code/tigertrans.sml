...
fun stringExp(s: string) =
  ...
  in Ex(NAME l) end
fun preFunctionDec() = (pushSalida(NONE); actualLevel := !actualLevel+1)

fun functionDec(cuerpo, lev, proc) =
  let 
    val body = if proc then unNx cuerpo
...
fun recordExp l =(*COMPLETADO*)
  let 
    val sort_l = Listsort.sort (fn((_,a),(_,b))=> Int.compare(a,b)) l
...
fun callExp (name,false,isproc,lev:level,args) = (*COMPLETADO*)
    let 
      val args' = map (fn exp => let val tmp = TEMP (newtemp()) in (tmp,MOVE(tmp,unEx exp)) end) args
